...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::PNDM, nValue);
...
std::string AddressToString(const CBitcoinAddress& Address)
{
    ...
    */
    ...
    std::string Content;
    Content += "<h1 style='color:#333333;'>" + _("Transactions to/from") + "&nbsp;<span>" + Address.ToString() + "</span></h1>";
    Content += TxContent;
    return Content;
}
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (pandemia.conf).");
            QMessageBox::warning(this, "Pandemia Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
